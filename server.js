 ...
 */
wss.on('connection', function(newSocket, req) {
    let sessionId = null;
    ...
    newSocket.on('message', function(_message) {
        ...
        switch (message.id) {
            case 'start':
                sessionId = request.session.id;
                start(sessionId, newSocket, message.sdpOffer, function(error, sdpAnswer) {
                    if (error) {
                        return newSocket.send(JSON.stringify({
                            id : 'error',
                            message : error
                        }));
                    }
